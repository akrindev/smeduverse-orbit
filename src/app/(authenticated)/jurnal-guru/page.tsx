...
import { useTeacherJournalsQuery } from "@/queries";
...
export default function TeacherJournalPage() {
	...
	// Get current user and teacher journals from React Query
	const { user: currentUser } = useAuth();

	// Build filters for query
	const filters = selectedTeacher
		? {
	...
	const { data: journalsResponse, isLoading } =
		useTeacherJournalsQuery(filters);
	...
	return (
		<div className="flex flex-col h-full">
			...
			{isLoading ? (
				...
				<>
					<TeacherJournalTable journals={journalsResponse?.data || []} />

					{/* Pagination */}
					<div className="flex justify-center mt-4">
						<PaginationControls
							currentPage={journalsResponse?.current_page || 1}
							lastPage={journalsResponse?.last_page || 1}
