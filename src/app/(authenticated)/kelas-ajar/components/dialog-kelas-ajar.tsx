...
import { useCreateKelasAjarMutation } from "@/queries/useKelasAjarQuery";
...
export default function DialogKelasAjar() {
	// loading state
	...
	const createMutation = useCreateKelasAjarMutation();
	...
	const handleSave = useCallback(async () => {
		...
		try {
			...
			toast({
				title: "Berhasil",
				description: "Kelas ajar berhasil dibuat",
			});
			setOpen(false);
			setSelected({
				rombongan_belajar_id: null,
			...
			toast({
				title: "Error",
				description: "Gagal membuat kelas ajar",
				variant: "destructive",
			});
		} finally {
			setIsLoading(false);
		...
		<Dialog open={open} onOpenChange={setOpen}>
			<DialogTrigger asChild>
				<Button className="flex items-center">
					...
					Buat Kelas Ajar Baru
				</Button>
			</DialogTrigger>
			<DialogContent>
				<DialogHeader>
					<DialogTitle>Buat Kelas Ajar Baru</DialogTitle>
				</DialogHeader>
