...
export default function PenilaianClient({ modulUuid }: { modulUuid: string }) {
	...
	return (
		<div className="flex flex-col gap-3 mx-auto py-3">
			<div className="flex items-center gap-2">
				<Link href={`/kelas-ajar/assignment/recap/${modulUuid}`}>
				...
				<Button
					...
					className="flex items-center gap-2"
				>
					<PlusCircle className="w-4 h-4" /> Buat
				</Button>
			</div>
			<Card>
				<CardHeader className="flex flex-row justify-between items-center">
					<div>
						...
						<CardDescription>Tugas/penilaian pada kelas ajar ini</CardDescription>
					</div>
				</CardHeader>
				<CardContent>
					{listQuery.isLoading ? (
				...
				<TableBody>
					{table.getRowModel().rows.map((row) => (
						<TableRow
							...
							onClick={() =>
								router.push(
									`/kelas-ajar/${modulUuid}/penilaian/${row.original.uuid}`,
								)
							}
