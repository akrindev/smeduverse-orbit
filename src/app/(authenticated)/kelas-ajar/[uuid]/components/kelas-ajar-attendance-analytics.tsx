...
import { useAttendanceRecapQuery } from "@/queries/useAttendanceQuery";
import type { OrbitPresence } from "@/types/attendance";
...
export default function KelasAjarAttendanceAnalytics({
	modulUuid,
}: {
	modulUuid: string;
}) {
	const { data: rows = [], isLoading } = useAttendanceRecapQuery(modulUuid);

	const { sessions, totals, maxValue, totalAll } = useMemo(() => {
		const sessionMap = new Map<
			string,
			{ meta: OrbitPresence; counts: Counts }
		>();
	...
	const presentRate =
		totalAll > 0 ? Math.round((totals.h / totalAll) * 100) : 0;
