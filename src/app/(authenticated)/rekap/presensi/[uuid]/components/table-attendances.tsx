		...
		() => [
			...
			...orbitPresence.map((presence, i) => ({
				...
				cell: (cell) => {
					...
					const status = (
						matchedPresence?.presence?.status || ""
					).toUpperCase();
					...
					return (
						<div className={`${matchedColor} font-medium`}>{displayStatus}</div>
					);
				},
			})),
			{
							...
							{Object.entries(cell.row.original?.status_count || {}).map(
								([key, value]) => {
									...
									return (
										<Badge
											variant={"outline"}
											className={`text-white ${matchedColor}`}
											key={key}
										>
		...
		getRecapAttendances(modulUuid)
			.then((res) => {
				if (res.status === 200) {
					...
					const unifiedPresence = Array.from(presenceMap.values()).sort(
						(a, b) => {
...
function AttendanceDetailGrid({
	...
	columns,
	modulUuid,
}: AttendanceDetailTableProps & { modulUuid?: string }) {
	return (
							...
							{attendance.orbit_presence &&
								...
								(() => {
									const latest = attendance.orbit_presence
										.filter(
											(p: OrbitPresence) =>
												!modulUuid || p.orbit_modul_uuid === modulUuid,
										)
										.sort(
											(a: OrbitPresence, b: OrbitPresence) =>
									...
									return latest ? (
										<p>
											Terakhir presensi:
											{format(
												new Date(latest.presence.created_at),
												"dd MMMM yyyy",
												{ locale: id },
											)}
										</p>
